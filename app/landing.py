...
<section class="recorded-demo" aria-label="Recorded runs">
<div class="label-row">
<div><b>Recorded runs</b><div class="hint">Real executions captured from the deployed system. No key or inference call required.</div></div>
<span class="saved ok">free to explore</span>
</div>
<div class="examples">
<button class="example recorded-example" data-recording="revenue-by-country" type="button">Top 5 countries by revenue</button>
<button class="example recorded-example" data-recording="gold-preview" type="button">Preview a Gold mart</button>
<button class="example recorded-example" data-recording="missing-values" type="button">Missing-value analysis</button>
<button class="example blocked recorded-example" data-recording="blocked-pg-catalog" type="button">? Watch a guardrail fire</button>
</div>
<p class="hint">Recorded run ? request a demo key below to execute queries or ingest your own API live.</p>
</section>
...
<div id="relation-hint" class="hint">Try a recorded run above, or enter a demo key to query the live governed catalog.</div>
<div class="examples">
...
<div id="recorded-ids" class="recorded-ids" hidden></div>
...
$("question").addEventListener("input",()=>{bindCurrentRelation=true});
document.querySelectorAll(".ask-example").forEach(b=>b.onclick=()=>{const p=promptFor(b.dataset.kind);if(p){bindCurrentRelation=true;$("question").value=p;$("question").focus()}else relationHint.textContent="No Gold relation loaded yet. Ingest an API first, or type a relation name manually."});$("blocked-example").onclick=()=>{bindCurrentRelation=false;$("question").value=promptFor("blocked");$("question").focus()};
async function loadRecordedRun(slug){
if(runBtn.disabled)return;
resetOutput();
const token=viewToken;
$("run-title").textContent="Recorded SQL run";
runStatus.textContent="recorded";
placeholder.innerHTML="<div><b>Loading recorded run...</b></div>";
runCard.scrollIntoView({behavior:"smooth",block:"start"});
try{
const response=await fetch("/demo/recorded/"+encodeURIComponent(slug),{cache:"no-store"});
if(!response.ok)throw new Error("Recorded run could not be loaded.");
const recorded=await response.json();
if(token!==viewToken)return;
renderStages(recorded.stages||[]);
renderFinal({mode:"ask",status:recorded.status,result:recorded.result||{},guardrail:recorded.guardrail});
runStatus.textContent="recorded ? "+recorded.status;
elapsed.textContent=((recorded.elapsed_ms||0)/1000).toFixed(1)+"s";
recordedIds.textContent="Recorded execution ? Request ID: "+recorded.request_id+" ? Run ID: "+recorded.run_id;
recordedIds.hidden=false;
}catch(error){
if(token!==viewToken)return;
runStatus.className="status failed";
runStatus.textContent="failed";
placeholder.style.display="none";
answerTitle.style.display="block";
answer.textContent=error.message||"Recorded run could not be loaded.";
}
}
document.querySelectorAll(".recorded-example").forEach(button=>{
button.addEventListener("click",()=>loadRecordedRun(button.dataset.recording));
});
...
function renderGuard(g){if(!g)return;guard.classList.add("visible");guard.classList.toggle("info",g.kind==="catalog_mismatch");guardTitle.textContent=g.title||"Guardrail triggered";guardMessage.textContent=g.message||"The run was blocked safely."}
...
runBtn.onclick=async()=>{const k=key.value.trim();if(!k){key.focus();saved.textContent="key required";return}let path,payload;if(mode==="ask"){const q=$("question").value.trim();if(!q){$("question").focus();return}path="/demo/ask";payload={question:q,relation:bindCurrentRelation?(currentRelation||null):null}}else{const u=$("api-url").value.trim(),g=$("gold-goal").value.trim();if(!u){$("api-url").focus();return}if(!g){$("gold-goal").focus();return}path="/demo/ingest";payload={api_url:u,gold_goal:g}}runBtn.disabled=true;$("run-title").textContent=mode==="ask"?"SQL analyst run":"ETL analyst run";resetOutput();startClock();try{const resp=await fetch(path,{method:"POST",headers:{...auth(),"Content-Type":"application/json"},body:JSON.stringify(payload)});let p;try{p=await resp.json()}catch(e){p={detail:"Non-JSON response"}}if(!resp.ok)throw new Error(typeof p.detail==="string"?p.detail:"Could not start run.");await poll(p.run_id)}catch(e){placeholder.style.display="none";runStatus.className="status failed";runStatus.textContent="failed";answerTitle.style.display="block";answer.textContent=e.message||"The request failed."}finally{stopClock();runBtn.disabled=false}};
askLatest.onclick=()=>{bindCurrentRelation=true;setMode("ask");$("question").value=promptFor("count");$("question").focus();window.scrollTo({top:0,behavior:"smooth"})};
</script>
</body></html>"""
    )
